...
EnterBox::EnterBox(int x, int y, int w, int h, SquareUnit *s, SquareUnit *s2)
    : Fl_Box(x,y,w,h)
    , isable{true}
    , square{s}
    , square2{s2}
    ...
    switch (e)
    {
        ...
        case FL_PUSH:
            ...
            case FL_LEFT_MOUSE:
                fl_register_images();
                if(w() > h()){
                    eBox_pic = Fl_Shared_Image::get("Img/work_1.PNG");
                }
                else{
                    eBox_pic = Fl_Shared_Image::get("Img/work_2.PNG");
                }
