...
const int MODE1 = 1 ;                       //正方形

const int MODE2 = 2 ;                       //六边形
...
void deafult_run(Fl_Widget *a,Fl_Window* w){
    ...
    w->~Fl_Window();
    sae ->game_start(DEAFULT,DEAFULT,MODE1);
    w->end();
}
...
void customize_run(Fl_Widget *a,Fl_Window* w){
    ...
    StartAndEnd* sae = new StartAndEnd;
    w->~Fl_Window();
    sae->game_start(sirow->value(),sicol->value(),sitype->value());
    w->end();
}
...
void start_game()
{
    Back_Window* w = new Back_Window(370,400,"Game",1);
    wiz = new Fl_Wizard(0,0,370,400," ");
    {
    ...
    {
        Fl_Group* Gr = new Fl_Group(0,0,370,400," ");
        NewButton* bac = new NewButton(100, 340, 150, 36, 4);//创建back按钮
        ...
        sicol->value(4);
        sitype = new SliderInput(140,180,80,30,"type:" );
        sitype->bounds(MODE1,MODE2);
        sitype->value(1);
        NewButton* Start = new NewButton(100, 270, 150, 36, 0);//创建Start按钮
